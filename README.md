...
应用已部署在内网 L40S 服务器，办公网通过端口映射访问同一服务：

| 访问位置       | 地址                         |
| -------------- | ---------------------------- |
| 内网 L40S      | <http://10.120.65.10:5173/>  |
| 办公网映射入口 | <http://101.89.57.55:18005/> |

TCP 端口映射关系：`101.89.57.55:18005 → 10.120.65.10:5173`。网页、实时录音 WebSocket、音频和文本下载共用这一入口。

服务器使用 `asr-web` systemd 服务运行并开机自启，录音及对应文本保存在 L40S 的 `/var/lib/asr/recordings`。上述地址当前使用 HTTP；浏览器麦克风录音需要可信 HTTPS，配置方法见 [服务器部署说明](deploy/README.md)。
